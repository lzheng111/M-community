Page({
  ...
  onShow(){
    ...
    wx.p.request({
      url:'http://localhost:3000/api/check/pay',
      method:'POST',
      data:{
        tel
      }
  ...
  handleInputChange(e){
    ...
    let searchList = null
      if(value){
      searchList = this.data.array.filter(item =>item.content.indexOf(value) >-1 || item.state.indexOf(value) >-1 || item.createdAt.indexOf(value) >-1 || item._id.indexOf(value) >-1) 
    }
    this.setData({

Page({
  ...
  onLoad(){
    ...
    wx.p.request({
      url:'http://localhost:3000/api/check/retroaction',
      method:'POST',
      data:{
        tel
      }
  ...
  handleInputChange(e){
    ...
    let searchList = null
      if(value){
      searchList = this.data.array.filter(item =>item.type.indexOf(value) >-1 || item.createdAt.indexOf(value) >-1 || item._id.indexOf(value) >-1 || item.content.indexOf(value) >-1) 
    }
    this.setData({
   ...
   let that = this
    wx.showModal({
      ...
      success:function(res) {
        if (res.confirm) {
         wx.p.request({
           url:'http://localhost:3000/api/delete/retroaction',
           method:'POST',
           data:{
             _id
           }
